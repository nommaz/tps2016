...
namespace HuginWS
{
    ...
    {
        [OperationContract]
        [WebInvoke(UriTemplate = "salepost?okc_id={okc_id}&password={password}", Method = "POST",
            RequestFormat = WebMessageFormat.Json,
           ResponseFormat = WebMessageFormat.Json)]
        string salepost(string okc_id, string password, Stream strPost)
        {
            ...
            try
            {
                ...
                StreamReader reader = new StreamReader(strPost);

                string salesinfo = "";
                salesinfo = reader.ReadToEnd();
                //salesinfo = OperationContext.Current.RequestContext.RequestMessage.ToString();
                //salesinfo = System.Text.Encoding.UTF8.GetString(OperationContext.Current.RequestContext.RequestMessage.GetBody<byte[]>());
